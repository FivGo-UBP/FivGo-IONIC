...
export class ActiveOrderPage implements OnInit, OnDestroy, AfterViewInit {
  ...
  async initMap() {
    ...
    // Read last known location from localStorage as a fallback to prevent map jumping from Jakarta
    const savedLatStr = localStorage.getItem('lastDriverLat');
    const savedLngStr = localStorage.getItem('lastDriverLng');
    const savedLat = savedLatStr ? parseFloat(savedLatStr) : null;
    const savedLng = savedLngStr ? parseFloat(savedLngStr) : null;

    let initLat = this.lastDriverLat !== null ? this.lastDriverLat : (savedLat !== null ? savedLat : -6.301);
    let initLng = this.lastDriverLng !== null ? this.lastDriverLng : (savedLng !== null ? savedLng : 107.303);
    ...
    // Asynchronously fetch actual GPS coordinates in the background to avoid blocking map render
    Geolocation.getCurrentPosition({ timeout: 3000 }).then((pos) => {
      if (pos && pos.coords) {
        this.lastDriverLat = pos.coords.latitude;
        this.lastDriverLng = pos.coords.longitude;
        if (this.map && this.mapReady && !this.order) {
          this.map.easeTo({
